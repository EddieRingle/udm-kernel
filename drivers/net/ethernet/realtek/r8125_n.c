                                  ...
                                  struct ethtool_pauseparam *pause)
{
        ...
        if ((pause->tx_pause) && (!pause->rx_pause))
				newfc = rtl8125_fc_tx_pause;
		else if ((!pause->tx_pause) && (pause->rx_pause))
				newfc = rtl8125_fc_rx_pause;
		else if ((pause->tx_pause) && (pause->rx_pause))
        ...
        if (pause->autoneg != tp->autoneg)
                tp->autoneg = pause->autoneg;
...
rtl8125_hw_config(struct net_device *dev)
{
	...
	u32 tx_dma_burst = TX_DMA_BURST_UNLIM;
#ifdef CONFIG_R8125_ALPINE_TX_DMA_QUIRK_CHECK
	const int crooked_domain = 2;
	if (pdev->bus && pci_domain_nr(pdev->bus) == crooked_domain) {
		tx_dma_burst = TX_DMA_BURST_WA;
		netdev_warn(dev, "pci_domain[%d]: TX_DMA_BURST workaround engaged", pci_domain_nr(pdev->bus));
	}
	netdev_warn(dev, "Using TX_DMA_BURST[%d]", tx_dma_burst);
#endif
